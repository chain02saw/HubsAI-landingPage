...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const userDisplayName = profileData?.fullName || user?.name || "Gamer";
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-400">Brand Partners</p>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-6">
        ...
        >
          {tabs.map((tab, index) => (
            ...
            >
              ...
              {/* Tooltip */}
              <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-1 bg-slate-700 text-slate-300 text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
                {tab.description}
              </div>
            </motion.button>
          ))}
        ...
        {/* Tab Content */}
        <motion.div
          className="bg-slate-800 border border-slate-700 rounded-2xl p-6 min-h-96"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
